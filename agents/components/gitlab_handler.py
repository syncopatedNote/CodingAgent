...
class GitLabHandler:
    ...
    async def load_development_rules(self, state: BaseState) -> Command:
        ...
        try:
            async with multi_server_mcp_client.session("gitlab") as session:
                ...
                get_tree_tool = next((tool for tool in tools if "get_repository_tree" in tool.name), None)
                get_file_tool = next((tool for tool in tools if "get_file_contents" in tool.name), None)
                    ...
                    return Command(
                        ...
                        update={
                            ...
                            "messages": state["messages"] + [
                                AIMessage(content=f"Successfully loaded development rules from {len(md_files_content)} .md files")
                            ]
                        }
                    )
                else:
    ...
    async def create_branch_and_commit(state: BaseState) -> Command:
        ...
        try:
            ...
            feature_desc = await GitLabHandler._extract_feature_description_from_confluence(state)
            
            # If no Confluence title available, fall back to Jira ticket summary
            if not feature_desc:
                feature_desc = GitLabHandler._extract_feature_description_from_jira(state)
    ...
    async def _extract_feature_description_from_confluence(state: BaseState) -> str:
        ...
        try:
            ...
            # If any error occurs, return empty string to fall back to Jira summary
            pass
    ...
    def _format_title_for_branch(title: str) -> str:
        ...
        words = [word for word in clean_title.split() if word.strip() and word not in stop_words]
    ...
    async def commit_code(self, state: BaseState) -> Command:
        ...
        try:
            async with multi_server_mcp_client.session("gitlab") as session:
                ...
                create_branch_tool = next((tool for tool in tools if "create_branch" in tool.name), None)
                create_file_tool = next((tool for tool in tools if "create_or_update_file" in tool.name), None)
            ...
            return Command(
                ...
                update={
                    ...
                    "messages": state["messages"] + [
                        AIMessage(content=f"Successfully committed code to branch: {state['branch_name']}")
                    ]
                }
            )
    ...
    def _is_md_file(self, item):
        ...
        if isinstance(item, dict):
            ...
            item_path = item.get("path", "")
            return (item_type == "blob" or item_type == "file") and item_path.lower().endswith('.md')
        return False
