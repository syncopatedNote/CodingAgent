...
supervisor → tool-executor loop, so the genuinely-identical, fiddly bits live
here as plain functions (no base class, no hooks):

* ``prune_tool_cycles``      — cap accumulated tool-call cycles to keep context
                               small and Bedrock's toolUse/toolResult pairing intact.
* ``execute_tool_with_retry`` — run a single tool call with the retry ladder
                               (no retry on bad input, back-off on transient errors).
* ``invoke_llm_with_retry``  — call a tool-bound LLM with retries on transient
                               model errors.
"""
...
async def invoke_llm_with_retry(
    llm_with_tools, messages: list[BaseMessage], max_retries: int = 3
) -> AIMessage:
    """Invoke a tool-bound LLM, retrying transient model errors with back-off.

    Returns the model's ``AIMessage`` on success. On exhaustion returns a
    plain ``AIMessage`` describing the failure so the caller can surface it
    ...
    for attempt in range(max_retries):
        try:
            ...
            exc_repr = repr(exc)
            is_retryable = any(s in exc_repr for s in RETRYABLE_MODEL_ERRORS)
    ...
    logger.error(f"LLM call failed: {last_exc}", exc_info=True)
    return AIMessage(
        content=(
            ...
            "the LLM service is configured and running."
        )
    )
...
async def execute_tool_with_retry(
    ...
    - ``GraphInterrupt`` propagates (it is how ``ask_user`` pauses the graph).
    - ``ValueError`` (bad input) fails immediately — retrying won't help.
    - ``ConnectionError`` / ``TimeoutError`` retry with exponential back-off.
    - Any other exception retries once, then fails.

    On exhaustion the returned string is prefixed with ``TERMINAL TOOL FAILURE``
    so the supervisor LLM knows not to retry the call.
    """
    ...
    for attempt in range(max_attempts):
        try:
            ...
            logger.error(f"Non-retryable error for tool '{name}': {exc}", exc_info=True)
            return (
                f"TERMINAL TOOL FAILURE — '{name}' rejected the request: {exc}. "
            ...
            if attempt < max_attempts - 1:
                ...
                await asyncio.sleep(delay)
                continue
            logger.error(f"Tool '{name}' exhausted retries: {exc}", exc_info=True)
            content = (
                f"TERMINAL TOOL FAILURE — '{name}' failed after {max_attempts} "
                f"attempts due to a connection or timeout error: {exc}. "
            ...
            if attempt == 0:
                ...
                continue
            logger.error(f"Tool '{name}' failed after retry: {exc}", exc_info=True)
            content = (
                f"TERMINAL TOOL FAILURE — '{name}' encountered an unexpected "
                f"error: {exc}. Do NOT retry this tool call."
            )
            break
