...
class JiraResponse(BaseModel):
    ...
    labels: List[str] = Field(
        default_factory=list, description="Labels associated with the issue"
    )
    ...
    issue_type: Optional[Dict[str, Any]] = Field(
        None, alias="issuetype", description="Issue type information"
    )
    resolution: Optional[Dict[str, Any]] = Field(
        None, description="Resolution information if resolved"
    )
    components: Optional[List[Dict[str, Any]]] = Field(
        None, description="Components associated with the issue"
    )
    fix_versions: Optional[List[Dict[str, Any]]] = Field(
        None, alias="fixVersions", description="Fix versions"
    )
    affects_versions: Optional[List[Dict[str, Any]]] = Field(
        None, alias="versions", description="Affected versions"
    )

    # Time tracking
    time_estimate: Optional[int] = Field(
        None, alias="timeestimate", description="Time estimate in seconds"
    )
    time_original_estimate: Optional[int] = Field(
        None,
        alias="timeoriginalestimate",
        description="Original time estimate in seconds",
    )
    time_spent: Optional[int] = Field(
        None, alias="timespent", description="Time spent in seconds"
    )

    # Dates
    due_date: Optional[str] = Field(
        None, alias="duedate", description="Due date if set"
    )
    resolution_date: Optional[str] = Field(
        None, alias="resolutiondate", description="Resolution date if resolved"
    )

    # Comments and attachments (if expanded)
    comments: Optional[Dict[str, Any]] = Field(None, description="Comments if expanded")
    attachments: Optional[List[Dict[str, Any]]] = Field(
        None, description="Attachments if expanded"
    )

    # Workflow
    transitions: Optional[List[Dict[str, Any]]] = Field(
        None, description="Available transitions if expanded"
    )

    # Custom fields - using a flexible approach for the many custom fields
    custom_fields: Optional[Dict[str, Any]] = Field(
        None, description="Custom fields with dynamic keys"
    )

    # Subtasks and links
    subtasks: Optional[List[Dict[str, Any]]] = Field(
        None, description="Subtasks if present"
    )
    issue_links: Optional[List[Dict[str, Any]]] = Field(
        None, alias="issuelinks", description="Issue links if present"
    )

    # Epic information
    epic_link: Optional[str] = Field(
        None, description="Epic link if issue is part of an epic"
    )
    epic_name: Optional[str] = Field(None, description="Epic name if this is an epic")

    # Sprint information (for Agile boards)
    sprint: Optional[Union[str, List[Dict[str, Any]]]] = Field(
        None, description="Sprint information"
    )
    ...
    security: Optional[Dict[str, Any]] = Field(
        None, description="Security level if set"
    )
    ...
    aggregate_progress: Optional[Dict[str, Any]] = Field(
        None, alias="aggregateprogress", description="Aggregate progress"
    )
...
class JiraSearchResponse(BaseModel):
    ...
    expand: Optional[str] = Field(
        None, description="Fields that were expanded in the response"
    )
    start_at: int = Field(0, alias="startAt", description="Starting index of results")
    max_results: int = Field(
        50, alias="maxResults", description="Maximum results per page"
    )
...
class JiraErrorResponse(BaseModel):
    ...
    error_messages: Optional[List[str]] = Field(
        None, alias="errorMessages", description="General error messages"
    )
